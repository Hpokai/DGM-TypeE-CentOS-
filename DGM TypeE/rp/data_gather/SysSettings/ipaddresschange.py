...
if __name__ == '__main__':
    ...
    if Path(path).exists():
        ...
        try:
            ret = check_output(['sudo', 'ip', 'addr', 'add', '{}/{}'.format(network['eth0_ip'], network['eth0_mask']), 'broadcast', network['eth0_broad'], 'dev', 'eth0'])
        except CalledProcessError as err:
            print('CalledProcessError:', err)
        else:
            try:
        ...
        try:
            ret = check_output(['sudo', 'ip', 'addr', 'add', '{}/{}'.format(network['eth1_ip'], network['eth1_mask']), 'broadcast', network['eth1_broad'], 'dev', 'eth1'])
        except CalledProcessError as err:
            print('CalledProcessError:', err)
        else:
            try:
        ...
        try:
            ret = check_output(['sudo', 'route', 'add', 'default', 'gw', network['eth1_gateway'], 'eth1'])
        except CalledProcessError as err:
            print('CalledProcessError:', err)
